...
[FriendlyName("Switch Camera", "Switches to the Target GameObject's camera.")]
public class uScriptAct_SwitchCamera : uScriptLogic
{
   ...
   public void In(
      [FriendlyName("Target", "The GameObject containing the camera to switch to.")]
      GameObject Target,
      
      [FriendlyName("Enable AudioListener", "Whether or not to enable that camera's AudioListener component (if it has one).")]
      [SocketState(false, false)]
      bool EnableAudioListener,
      
      [FriendlyName("Disable Other Cameras", "Disable Other Cameras: Whether or not to disable all other cameras.")]
      [SocketState(false, false)]
      bool DisableOtherCameras
      )
   {
      if ( Target != null)
      {
