...
[FriendlyName("Access List (Color)", "Access the contents of a list. May return the first or last item, a random item, or the item at a specific index.")]
public class uScriptAct_AccessListColor : uScriptLogic
{
   ...
   public void First(Color[] List, int Index, out Color Value)
   {
      Value = List[0];
   }

   // Parameter Attributes are applied below in AtIndex()
   public void Last(Color[] List, int Index, out Color Value)
   {
      Value = List[List.Length - 1];
   }

   // Parameter Attributes are applied below in AtIndex()
   public void Random(Color[] List, int Index, out Color Value)
   {
