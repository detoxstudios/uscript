...
namespace Detox.ScriptEditor
{
   public partial class ScriptEditorCtrl : ToolWindow
   {
      ...
      public void CenterOnPoint(Point point)
      {
         ...
         point.X = Math.Min(0, Math.Max(-System.Int32.MaxValue, -point.X + (int)(uScript.Instance.NodeWindowRect.width * 0.5f)));
         point.Y = Math.Min(0, Math.Max(-System.Int32.MaxValue, -point.Y + (int)(uScript.Instance.NodeWindowRect.height * 0.5f) - (int)uScript.Instance.NodeToolbarRect.height));
      ...
      public void CenterOnNode(Node node)
      {
         ...
         m_FlowChart.Location = new Point( Math.Min(0, Math.Max(-System.Int32.MaxValue, -center.X + halfWidth)),
                                           Math.Min(0, Math.Max(-System.Int32.MaxValue, -center.Y + halfHeight - (int)uScript.Instance.NodeToolbarRect.height)));
         m_FlowChart.Invalidate();
      }
      ...
      public void RefreshScript( List<Guid> guidsToSelect, bool zoomExtents, Point location )
      {
         ...
         if (m_FlowChart.Nodes.Length > 0)
         {
            ...
            else if (zoomExtents)
            {
               ...
               m_FlowChart.Location = new Point(Math.Min(0, Math.Max(-System.Int32.MaxValue, -center.X + halfWidth)), Math.Min(0, Math.Max(-System.Int32.MaxValue, -center.Y + halfHeight - (int)uScript.Instance.NodeToolbarRect.height)));
               m_FlowChart.Invalidate();
            }
         }
